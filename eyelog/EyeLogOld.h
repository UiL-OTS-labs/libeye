...
/*
 * Swig doesnt like DArrays with object without a default constructor.
 */
#ifndef SWIG
        /**
         * Return a DArray with all logentries.
         */
        DArray<EyeLogEntry> getEntries()const
        {
            eyelog_entry **entries=NULL;
            unsigned size;
            eye_log_get_entries(mlog, &entries, &size);
            DArray<EyeLogEntry> ret;
            ret.reserve(size);
...
#else
        /**
         * Return a DArray with shared_ptr
         */
        DArray<std::shared_ptr<EyeLogEntry> > getEntries() const
        {
            eyelog_entry **entries=NULL;
            unsigned size;
            eye_log_get_entries(mlog, &entries, &size);
            DArray<std::shared_ptr<EyeLogEntry> > ret;
            ret.reserve(size);
            for (unsigned i=0; i<size; i++) {
                ret.push_back(
